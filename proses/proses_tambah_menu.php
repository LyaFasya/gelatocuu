...
if($statusUpload == 0){
    ...
    if (move_uploaded_file($_FILES['foto_menu']['tmp_name'], $target_file)){
        $query = mysqli_query($conn, "INSERT INTO menu (foto_menu,nama_menu,keterangan,kategori,harga,stok) values ('".$kode_rand.$_FILES['foto_menu']['name']."','$nama_menu','$keterangan','$kategori','$harga','$stok')");
    if($query){
